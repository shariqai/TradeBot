...
def pairs_trading(stock1, stock2, data):
    ...
    """
    # Ensure data is a DataFrame and contains stock1 and stock2 columns
    ...
    if stock1 in data.columns and stock2 in data.columns:
        ...
        return (spread.iloc[-1] - mean) / std_dev  # Using .iloc[-1] for the last value
    else:
...
def momentum_trading(data, lookback=14):
    ...
    """
    # Ensure data is a pandas Series or DataFrame and check for enough rows
...
def mean_reversion(data, lookback=14):
    ...
    if len(data) >= lookback:
        ...
        return (data - moving_avg).dropna()  # Subtract the moving average and drop NaN values
    else:
